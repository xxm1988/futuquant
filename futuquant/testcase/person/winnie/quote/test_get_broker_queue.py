...
class GetBrokerQueue(unittest.TestCase):
    ...
    def common_get_broker_queue(self, code, case_name):
        ...
        print(case_name, file=f, flush=True)
        ret_code, ret_data_big, ret_data_ask = quote_ctx.get_broker_queue(code)
        print(ret_data_big, ret_data_ask, file=f,flush=True)
    ...
    def test_get_broker_queue_hk_stock(self):
        ...
        self.assertEqual(len(ret_data_ask) > 0, True)
        self.assertEqual(len(ret_data_big) > 0, True)
...
class AsyncBroker(unittest.TestCase):
    ...
    def step_base(self, code_list, casename=sys._getframe().f_code.co_name ):
        ...
        #订阅经济队列
        quote_ctx.subscribe(code_list, SubType.BROKER)
...
class BrokerTest(BrokerHandlerBase):
    ...
    def on_recv_rsp(self, rsp_str):
        ...
        :return:
        '''
        ret_code, ret_data = super(BrokerTest, self).on_recv_rsp(rsp_str) # 基类的on_recv_rsp方法解包返回了经济队列信息
        #打印,记录日志
        print(ret_code, file=f, flush=True)
